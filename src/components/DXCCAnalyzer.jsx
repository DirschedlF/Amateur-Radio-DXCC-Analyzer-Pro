  ...
  return (
    <div ref={containerRef} className="container mx-auto px-4 py-8 max-w-[1800px]">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-center">Amateur Radio Logbook Analysis Tool v2.7.0</p>
        {logData && fileName && (
          <>
                ...
                <ResponsiveContainer width="100%" height={printMode !== 'none' ? 195 : 300}>
                  <BarChart data={chartData.continentData} margin={printMode !== 'none' ? { top: 5, right: 5, left: -10, bottom: 5 } : { top: 5, right: 20, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value, name) => [value, name === 'confirmed' ? 'Confirmed' : name === 'workedOnly' ? 'Worked Only' : name === 'notWorked' ? 'Not Worked' : name]}
                    />
                    <Legend formatter={(value) => value === 'confirmed' ? 'Confirmed' : value === 'workedOnly' ? 'Worked Only' : value === 'notWorked' ? 'Not Worked' : value} />
                    ...
                    {filterStatus === 'allentities' && <Bar dataKey="notWorked" stackId="a" fill="#4b5563" radius={[4, 4, 0, 0]} />}
                  </BarChart>
                </ResponsiveContainer>
              </div>
          ...
          <div className="bg-gray-800 rounded-lg p-4 mb-6 flex flex-col gap-3 print:hidden">
            ...
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                />
                {searchTerm && (
                  <X className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 cursor-pointer hover:text-white" onClick={() => { setSearchTerm(''); setCurrentPage(1) }} />
                )}
              </div>
            </div>
